...
abstract class StyleInterface {
		...
		foreach ( $this->generated as $device => $css ) {
			...
			if ( ! empty( $generated_device_style ) ) {
				if ( 'Desktop' === $device ) {
					...
					$generated_style[] = "@media only screen and (max-width: 1200px) { {$generated_device_style} }";
	...
	public function inject_style( $data ) {
		if ( isset( $data['value'] ) && $data['device_control'] && ! $this->is_variable( $data['value'] ) && is_array( $data['value'] ) ) {
	...
	public function handle_gradient( $props, $angle ) {
		...
		return "background-image: linear-gradient({$angle}deg, {$colors});";
	}
						...
						if ( isset( $value['type'] ) ) {
							if ( 'full' === $value['type'] ) {
								...
								if ( isset( $value['width'] ) ) {
									...
									return "{$unit_point} display: inline-block!important;";
								}
							}
						}
					},
	...
	public function handle_box_shadow( $value ) {
		if ( isset( $value['color'] ) ) {
			$position     = isset( $value['position'] ) && 'inset' === $value['position'] ? $value['position'] : '';
			$horizontal   = isset( $value['horizontal'] ) ? $value['horizontal'] : 0;
			$vertical     = isset( $value['vertical'] ) ? $value['vertical'] : 0;
			$blur         = isset( $value['blur'] ) ? $value['blur'] : 0;
			$spread       = isset( $value['spread'] ) ? "{$value['spread']}px" : '';
