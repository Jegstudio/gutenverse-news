...
const Slider5Block = compose(
    ...
    function RenderColumn(props) {
        ...
        return (
            <div ref={blockRef} className="gvnews_slider_type_5 gvnews_slider" data-autoplay={autoplay ? true : ''} data-delay={sliderDelay}>
                {content}
            </div>
        );
    }
    ...
    function resetblock() {
        ...
        if (postData.length > 0) {
            setBlock(
                <div key={Math.random().toString(36).substring(2)} className={`gvnews_slider_wrapper gvnews_slider_type_5_wrapper ${overlayOption == 'no' ? 'no-overlay' : ''}`}>
    ...
    return (
        <>
            {isDeprecated ? (
                <PanelUpgradePro title="Slider 5" />
