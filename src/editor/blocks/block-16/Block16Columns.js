...
const Block16Columns = props => {
    ...
    const RenderBlock1 = props => {
        const className = withFormatName('gvnews_post gvnews_pl_lg_5', props.post);
        return (
    ...
    const RenderBlock2 = props => {
        ...
        const className = withFormatName(
            `gvnews_post gvnews_pl_xs_2 ${isLoadMore && index >= loadValidAnim && index <= postDataLen && page > 1 ? `gvnews_ajax_loaded anim_${(index - loadValidAnim)}` : ''}`,
            post
        );
        return (
            <article className={className}>
                <i className="fas fa-caret-right"></i>
                <ContentModule title={true} meta={false} excerpt={false} read={false} post={post} attr={props.attr} />
            </article>
        );
    };
